...
if (n==1)
	{
	history="######1######"+console.dir(doc);
	}
else
	{
	history="######"+n+"######"+history+console.dir(doc);
	}
         console.log(history);
      }
   });
...
socket.emit('updatechat', 'SERVER',history);
});
...
// we store the username in the socket session for this client
		...
		if (pseudo && pseudo !="")
		{
		socket.emit('updatechat', 'SERVER', 'Vous êtes connecté,bienvenue <span class='+pseudo+'>'+pseudo+'</span>');
...
mess="<strong>"+time+"</strong><span style='color:"+color+"'><b>"+pseudo+"</b></span><em> "+message+"</em>";
